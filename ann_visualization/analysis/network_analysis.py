...
from ann_visualization.network.data_loader import *
from ann_visualization.network.network import *
...
def compare_projections(datatype, model_name, n_layer, label, size=2000):
    ...
    clf.fit(layer_bt[:size], label_test)
    plot_new_neuron_projection(x_bt, hue=clf.feature_importances_)
...
def show_seq_projections(datatype, model_name, n_layer, epoch, size=10000):
    ...
    if "mlp" in model_name:
        ...
        model_at = create_cnn(datatype)
        # model_at = load_model_from_file(model_name,epoch)
        load_weights_from_file(model_at, model_name, 100, epoch)
        layer_at = get_activations_cnn(model_at, X_test[:size], size)[n_layer - 1]
    ...
    # print(X_test.shape)
    # print(layer_at.shape)
    # print(Y_test.shape)

    Y_pred = predict_classes(model_at, X_test)
